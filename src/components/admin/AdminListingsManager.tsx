...
import type { Listing, Category, Image, User } from '@prisma/client'
...
export default function AdminListingsManager({ listings: initialListings }: AdminListingsManagerProps) {
  ...
  const handleDelete = async (id: number, title: string) => {
    if (!confirm(`Bạn có chắc muốn xóa tin "${title}"? (Admin action)`)) {
      return
    }
  ...
  const filteredListings = filterStatus === 'ALL'
    ? initialListings
    : initialListings.filter(listing => listing.status === filterStatus)
  ...
  return (
    <div>
      ...
      <div className="mb-6 flex gap-2">
        ...
        >
          Đang hoạt động ({initialListings.filter(l => l.status === 'ACTIVE').length})
        </button>
        <button
        ...
        >
          Đã bán ({initialListings.filter(l => l.status === 'SOLD').length})
        </button>
        <button
        ...
        >
          Đã ẩn ({initialListings.filter(l => l.status === 'HIDDEN').length})
        </button>
      </div>
            ...
            >
              <div className="flex gap-6">
                ...
                <div className="flex-1 min-w-0">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {listing.user && (
                        <div className="mb-2 text-sm text-gray-600">
                          <span className="font-medium">Người đăng:</span> {listing.user.name || listing.user.email}
                        </div>
                      )}
                      {listing.guestEmail && (
                        <div className="mb-2 text-sm text-gray-600">
                          <span className="font-medium">Guest:</span> {listing.guestEmail}
                        </div>
                      )}
                  ...
                  {/* Admin Actions */}
                  <div className="flex flex-wrap gap-2 mt-4 pt-4 border-t border-red-200">
